...
*/
#define lemonStrlen(X) (X ? ((int)strlen (X)) : 0)
...
static char *outputDir, *outputName;
...
void FindActions (struct lemon * lemp)
{
    ...
    ** start nonterminal.  */
    ...
    for (rp = lemp->rule; rp; rp = rp->next)
    {
        if (rp->canReduce)
            continue;
        dbg ("%s:%d: This rule can not be reduced.\n", lemp->filename,
             rp->ruleline);
    }
}
...
static void handle_d_option (char * z) { outputDir = strdup (z); }

static void handle_n_option (char * z) { outputName = strdup (z); }
...
int main (int argc, char ** argv)
{
    ...
    static struct s_options options[] = {
        ...
        {OPT_FSTR, "d", (char *)handle_d_option, "Output directory for files."},
        ...
        {OPT_FSTR, "n", (char *)handle_n_option, "Output base name."},
    ...
    {
        fprintf (stderr,
                 "Output name and directory must be specified like this:\n"
                 "-nbaseName -d/output/Dir/");
        exit (1);
    }
    ...
    *cp = '=';
    ...
    else
    {
        ...
        case OPT_FINT:
            ...
            break;
        case OPT_STR:
        case OPT_FSTR: sv = cp; break;
        }
        switch (op[j].type)
        {
        case OPT_FLAG:
        case OPT_FFLAG: break;
        case OPT_DBL: *(double *)(op[j].arg) = dv; break;
        case OPT_FDBL: (*(void (*) (double)) (op[j].arg)) (dv); break;
        case OPT_INT: *(int *)(op[j].arg) = lv; break;
        case OPT_FINT: (*(void (*) (int)) (op[j].arg)) ((int)lv); break;
        case OPT_STR: *(char **)(op[j].arg) = sv; break;
        case OPT_FSTR: (*(void (*) (char *)) (op[j].arg)) (sv); break;
        }
    }
    return errcnt;
}
...
PRIVATE char * file_makename (struct lemon * lemp, const char * suffix)
{
    ...
    name = (char *)malloc (lemonStrlen (outputDir) + lemonStrlen (outputName) +
                           lemonStrlen (suffix) + 5);
    if (name == 0)
    {
        fprintf (stderr, "Can't allocate space for a filename.\n");
        exit (1);
    }
    lemon_sprintf (name, "%s%s%s", outputDir, outputName, suffix);
    return name;
}
                 ...
                 )
{
    ...
    case ACCEPT: fprintf (fp, "%*s accept", indent, ap->sp->name); break;
    case ERROR: fprintf (fp, "%*s error", indent, ap->sp->name); break;
    case SRCONFLICT:
    case RRCONFLICT:
...
PRIVATE int translate_code (struct lemon * lemp, struct rule * rp)
{
                ...
                for (i = 0; i < rp->nrhs; i++)
                {
                    if (rp->rhsalias[i] && strcmp (cp, rp->rhsalias[i]) == 0)
                    {
                        ...
                        else if (cp != rp->code && cp[-1] == '@')
                        {
                             ...
                             *substituted * the token number of X, not the value
                             *of X */
    ...
    {
        dbg ("Label \"%s\" for \"%s(%s)\" is never used.\n", rp->lhsalias,
                  ...
                  )
{
    ...
    fprintf (out, "#define YYACTIONTYPE %s\n",
             minimum_size_type (0, lemp->nstate + lemp->nrule * 2 + 5,
                                &szActionType));
    lineno++;
    if (lemp->wildcard)
    {
