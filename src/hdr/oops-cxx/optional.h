...
#define optional_ALIGN_AS(unused) optional_FEATURE_ALIGN_AS
...
template <bool condition, typename Then, typename Else> struct select;
template <typename Then, typename Else> struct select<true, Then, Else>
{
    typedef Then type;
};
template <typename Then, typename Else> struct select<false, Then, Else>
{
    typedef Else type;
};
...
template <typename T> union storage_t {
  private:
    ...
    value_type * value_ptr () const { return as ((value_type *)0); }

    value_type * value_ptr () { return as ((value_type *)0); }

    value_type const & value () const { return *value_ptr (); }
...
#undef optional_ALIGN_AS
    ...
    template <typename U> U * as (U *) const
    {
        return reinterpret_cast<U *> (
            const_cast<aligned_storage_t *> (&buffer));
    }
};
...
template <typename T> class optional
{
  ...
  public:
    ...
    optional () : has_value (false), contained () {}

    optional (nullopt_t) : has_value (false), contained () {}

    optional (value_type const & value) : has_value (true), contained (value) {}

    optional (optional const & other) : has_value (other.initialized ())
    {
    ...
    {
        using std::swap;
        if (initialized () == true && other.initialized () == true)
        {
            swap (**this, *other);
        }
        else if (initialized () == false && other.initialized () == true)
        {
            initialize (*other);
            other.clear ();
        }
        else if (initialized () == true && other.initialized () == false)
        {
    ...
    value_type const & value () const
    {
        if (!initialized ())
            throw bad_optional_access (
                "accessing value of disengaged optional (const)");
  ...
  private:
    ...
    bool initialized () const { return has_value; }
...
template <typename T> void swap (optional<T> & x, optional<T> & y)
{
    x.swap (y);
}
